...
class SignInActivity : AppCompatActivity(), CoroutineScope {
    ...
    private fun checkAuthCodeExist() = authTokenProvider.token.isNullOrBlank().not()
    ...
    private fun getAccessToken(code: String) = launch(coroutineContext){
        try{
          withContext(Dispatchers.IO){
           ...
           e.printStackTrace()
            Toast.makeText(this@SignInActivity, "로그인 과정에서 에러가 발생했씁니다. : ${e.message}", Toast.LENGTH_SHORT).show()
         }
    }
